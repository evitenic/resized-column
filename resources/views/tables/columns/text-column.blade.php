...
<div
    ...
    @if (count($arrayState))
        ...
        <{{ $isListWithLineBreaks ? 'ul' : 'div' }} @class([
            ...
            @if ($isListWithLineBreaks && $isLimitedListExpandable) x-data="{ isLimited: true }" @endif>
                ...
                @if (filled($formattedState = $formatState($state)) &&
                    ...
                    <{{ $isListWithLineBreaks ? 'li' : 'div' }}
                        @if ($itemIsCopyable) x-on:click="
                            ...
                            " @endif
                        @if ($isListWithLineBreaks && $loop->iteration > $listLimit) x-cloak
                            x-show="! isLimited"
                            x-transition @endif
                        @class([
                            'w-full min-w-0' => $canTruncate,
                            ...
                            \Filament\Support\get_color_css_variables($color, shades: [400, 600], alias: 'tables::columns.text-column.item.container') => $isBulleted && !in_array($color, [null, 'gray']),
                        ])>
                        @if ($isBadge)
                        ...
                        @else
                            <div @class([
                                ...
                                @if ($icon && in_array($iconPosition, [IconPosition::Before, 'before']))
                                    <x-filament::icon :icon="$icon" :class="$iconClasses" :style="$iconStyles" />
                                @endif
                                ...
                                <span @class([
                                    ...
                                    \Filament\Support\get_color_css_variables($color, shades: [400, 600], alias: 'tables::columns.text-column.item.label') => !in_array($color, [null, 'gray']),
            ...
            @if ($limitedArrayStateCount ?? 0)
                <{{ $isListWithLineBreaks ? 'li' : 'div' }}>
                    @if ($isLimitedListExpandable)
                        <x-filament::link color="gray" tag="div" x-on:click.prevent.stop="isLimited = false"
                            x-show="isLimited">
                            {{ trans_choice('filament-tables::table.columns.text.actions.expand_list', $limitedArrayStateCount) }}
                        </x-filament::link>

                        <x-filament::link color="gray" tag="div" x-cloak x-on:click.prevent.stop="isLimited = true"
                            x-show="! isLimited">
